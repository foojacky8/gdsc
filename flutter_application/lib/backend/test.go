...
func main() {
	...
	go func() {
		t := time.Now()
		genesisBlock := Block{}
		genesisBlock = Block{0, t.String(), 0.0, 0.0, calculateHash(genesisBlock), ""}
...
func calculateHash(block Block) string {
	record := strconv.Itoa(block.Index) + block.Timestamp + fmt.Sprintf("%f", block.TotalPrice) + fmt.Sprintf("%f", block.TotalProfit) + block.PrevHash
	h := sha256.New()
	h.Write([]byte(record))
	hashed := h.Sum(nil)
